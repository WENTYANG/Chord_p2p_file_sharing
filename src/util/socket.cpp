...
int initializeServer(const string& portNum) {
  ...
  if (status != 0) {
    throw SocketException("Error: cannot get address info for host.");
  }
  ...
  if (socket_fd == -1) {
    throw SocketException("Error: cannot create socket");
  }
  ...
  if (status == -1) {
    throw SocketException("Error:cannot bind socket");
  }

  // listen to port and wait for connection
  status = listen(socket_fd, 100);
  if (status == -1) {
    throw SocketException("Error:cannot listen on socket");
  }
...
int serverAcceptConnection(int serverFd, string& clientIp) {
  ...
  if (client_connection_fd == -1) {
    printf("%s\n", strerror(errno));
    throw SocketException("Error: cannot accept connection on socket.");
  }
...
int clientRequestConnection(const string& hostName, const string& portNum) {
  ...
  int client_fd = socket(host_info_list->ai_family, host_info_list->ai_socktype,
                         host_info_list->ai_protocol);
  if (client_fd == -1) {
    throw SocketException("Error: cannot create socket.");
  }
  ...
  if (status == -1) {
    throw SocketException("Error: cannot connect to socket.");
  }
...
int getPortNum(int socketFd) {
  ...
  if (getsockname(socketFd, (struct sockaddr*)&sin, &len) == -1) {
    throw SocketException("Error: cannot getsockname.");
  }
...
void socketSendMsg(int socket_fd, const void* buf, int len) {
  if (send(socket_fd, buf, len, 0) < 0) {
    close(socket_fd);
    throw SocketException("fail to send msg.");
  }
}
...
string socketRecvMsg(int socket_fd) {
  ...
  if (len <= 0) {
    ...
    throw SocketException("fail to accept msg.");
  }
